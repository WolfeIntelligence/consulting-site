...
/* Record when an outcome was first reached. Google's offline conversion
   ...
   speed-to-lead clock: minutes from enquiry to first call back. */
...
/* Normalisation and hashing for enhanced conversions. Google's rules: email
...
/* The same person asking twice within a month is one lead, not two. Match on
   normalised phone or email for the same client; the earlier lead wins. */
...
async function notifyNewLead(lead, client) {
  ...
  const rows = [
    ...
    ['Note', lead.duplicateOf ? 'They have enquired before in the last 30 days — this is a repeat.' : ''],
  ].filter((r) => r[1]);
  ...
  const head = (lead.duplicateOf ? 'Repeat enquiry for ' : 'New enquiry for ') + biz;
  const text = [
    head + ':', '',
    ...rows.map((r) => r[0] + ': ' + r[1]), '',
  ...
  const payload = {
    from, to,
    subject: (lead.duplicateOf ? 'Repeat enquiry: ' : 'New enquiry: ') + (lead.name || 'someone') + (lead.service ? ' — ' + lead.service : ''),
    text, html,
  };
