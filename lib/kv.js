...
/* Field maps. Used for small side indexes — which old record became which
 * object — where a set would answer "have I seen this" but not "what is it
 * now". */
const hset = (key, field, value) => cmd('HSET', key, field, String(value));
const hget = async (key, field) => { try { return await cmd('HGET', key, field); } catch (e) { return null; } };
const hgetall = async (key) => {
  let flat;
  try { flat = await cmd('HGETALL', key); } catch (e) { return {}; }
  const out = {};
  if (Array.isArray(flat)) { for (let i = 0; i < flat.length; i += 2) out[flat[i]] = flat[i + 1]; }
  else if (flat && typeof flat === 'object') { for (const k of Object.keys(flat)) out[k] = flat[k]; }
  return out;
};
