/* One-tap outcome page. Reads the signed token from the URL, shows which
   enquiry it is about, and only changes anything when the button is pressed.
...
(function () {
  ...
  fetch('/api/lead-status?t=' + encodeURIComponent(t))
    ...
    .then(function (x) {
      if (!x.r.ok) return fail(x.r.status === 401 ? 'It has expired or was altered.' : 'The enquiry could not be found.');
      var l = x.d.lead;
      app.innerHTML =
        '<h1>Record: ' + esc(LABEL[set]) + '</h1>' +
        '<p>Confirm this for the enquiry below' + (l.business ? ' at ' + esc(l.business) : '') + '.</p>' +
        '<div class="card"><b>' + esc(l.name || 'Enquiry') + '</b>' +
        '<div class="sub">' + esc([l.service, l.phone].filter(Boolean).join(' · ')) + '</div>' +
        '<div class="sub">Enquired ' + esc(String(l.at || '').slice(0, 10)) + '</div>' +
      ...
      btn.onclick = function () {
        ...
        fetch('/api/lead-status', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ t: t, set: set }) })
          ...
          .then(function (y) {
            ...
            msg.className = 'done';
            msg.textContent = 'Saved. ' + (y.d.lead.name || 'The enquiry') + ' is now marked ' + stateOf(y.d.lead).toLowerCase() + '.';
          })
          .catch(function (e) {
